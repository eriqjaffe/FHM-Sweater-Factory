...
ipcMain.on("local-font", (event, arg) => {
  ...
  dialog
    ...
    .catch((err) => {
      console.log(err);
      (json.status = "error"), (json.message = err);
      event.sender.send("local-font-response", json);
    });
});
      ...
      if (!result.canceled) {
        Jimp.read(buffer, (err, fir_img) => {
          if (err) {
            ...
            var watermark = fs.readFileSync(
              __dirname + "/images/fhm_watermark.png",
              { encoding: "base64" }
            );
...
function checkForUpdate() {
  versionCheck(updateOptions, function (error, update) {
    ...
    if (update) {
      // print some update info if an update is available
      dialog
        .showMessageBox(null, {
          type: "question",
          message:
            ...
            "\r\n\r\nVersion " +
            update.name +
            " is now availble.  Click 'OK' to go to the releases page.",
          buttons: ["OK", "Cancel"],
        })
        .then((result) => {
    ...
    switch (deform) {
        ...
        async function arch() {
          try {
            ...
            centeredImage.scan(
              0,
              0,
              centeredImage.bitmap.width,
              centeredImage.bitmap.height,
              function (x, y, idx) {
                const radians =
                  ((x / centeredImage.bitmap.width) * 360 * Math.PI) / 180;
                ...
                const clampedYFloor = Math.max(
                  0,
                  Math.min(centeredImage.bitmap.height - 1, yFloor)
                );
                const clampedYCeil = Math.max(
                  0,
                  Math.min(centeredImage.bitmap.height - 1, yCeil)
                );

                const colorFloor = Jimp.intToRGBA(
                  centeredImage.getPixelColor(x, clampedYFloor)
                );
                const colorCeil = Jimp.intToRGBA(
                  centeredImage.getPixelColor(x, clampedYCeil)
                );
      ...
      case "arc":
        ...
        async function arc() {
          let image = await Jimp.read(buffer);
          image.autocrop();
          let result = await distortUnwrap(image, "Arc", [
            parseInt(amount / 4),
          ]);
          let tempImg = await new Jimp(
            result.bitmap.width * 4,
      ...
      case "bilinearUp":
        ...
        async function bilinearUp() {
          ...
          const result = await distortUnwrap(
            image,
            "Polynomial",
            controlPoints
          );
          const tempImg = await new Jimp(
            result.bitmap.width * 4,
            result.bitmap.height * 4
          );
          await tempImg.blit(result, 5, 5);
        ...
        async function archUp() {
          try {
            ...
            tempImage.scan(
              ...
              function (x, y, idx) {
                const radians =
                  (((x * 180) / image.bitmap.width) * Math.PI) / 180;
                const offsetY = amount * -1 * Math.cos(radians);
                const newY = y + offsetY;
                ...
                const clampedYFloor = Math.max(
                  0,
                  Math.min(image.bitmap.height - 1, yFloor)
                );
                const clampedYCeil = Math.max(
                  0,
                  Math.min(image.bitmap.height - 1, yCeil)
                );

                const colorFloor = Jimp.intToRGBA(
                  image.getPixelColor(x, clampedYFloor)
                );
                const colorCeil = Jimp.intToRGBA(
                  image.getPixelColor(x, clampedYCeil)
                );
    ...
    },
    {
      ...
      submenu: [
        ...
        },
        {
          ...
          click: async () => {
            await shell.openExternal(
              "https://github.com/eriqjaffe/FHM-Sweater-Factory"
            );
          },
        },
        {
